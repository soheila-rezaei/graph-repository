...
const CommentForm = ({slug}) => {
    ...
    const sendHandler=()=>{
        if (name && email && text){
            ...
            toast.warn("تمام پست هارو پر کنید ",{
                position:"top-center",
            })
        }
    }
    if (data){
        toast.success("اطلاعات ارسال و منتظر تایید است",{
            position:"top-center"
        })
    }
    ...
    return (
       <Grid container sx={{boxShadow:"rgba(0,0,0,0.1) 0px 4px 12px",borderRadius:4 ,py:1 ,mt:5}}>
           ...
           <Grid item xs={12} m={2}>
               <TextField label="نام کاربری" variant="outlined" sx={{width:"100%"}} value={name}
                          onChange={(e)=>setName(e.target.value)}
               />
           </Grid>
           <Grid item xs={12} m={2}>
               <TextField label="ایمیل" variant="outlined" sx={{width:"100%"}} value={email}
                          onChange={(e)=>setEmail(e.target.value)}
               />
           </Grid>
           <Grid item xs={12} m={2}>
               <TextField label="متن کامنت" variant="outlined" sx={{width:"100%"}} value={text}
